...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    if (sigprocmask(SIG_BLOCK, &set, NULL) == -1) {   //设置信号阻塞
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno,
                      "sigprocmask() failed");
    }

    sigemptyset(&set);   //   为什么又置空
...
ngx_worker_process_init(ngx_cycle_t *cycle, ngx_int_t worker)
{
    ...
    if (worker >= 0 && ccf->priority != 0) {   //?????  youxianji
    ...
    if (worker >= 0) {
        cpu_affinity = ngx_get_cpu_affinity(worker);   //  cpu qinheli  cpu weizhi
    ...
    if (sigprocmask(SIG_SETMASK, &set, NULL) == -1) {   // ？？？？ 设置信号掩码为null 当前进程不再 信号阻塞
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno,
                      "sigprocmask() failed");
    }
    ...
    if (ngx_add_channel_event(cycle, ngx_channel, NGX_READ_EVENT,
                              ngx_channel_handler)  // gei channel[1] add read  
        == NGX_ERROR)
    {
        /* fatal */
        exit(2);
