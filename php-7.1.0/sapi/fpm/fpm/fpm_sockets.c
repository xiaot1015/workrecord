...
static int fpm_socket_af_inet_listening_socket(struct fpm_worker_pool_s *wp) /* {{{ */
{
	...
	for (p = servinfo; p != NULL; p = p->ai_next) {
		inet_ntop(p->ai_family, fpm_get_in_addr(p->ai_addr), tmpbuf, INET6_ADDRSTRLEN);   //IP地址转换函数，可以在将IP地址在“点分十进制”和“二进制整数”之间转换
		if (sock < 0) {
...
int fpm_sockets_init_main() /* {{{ */
{
	...
	if (0 == fpm_array_init(&sockets_list, sizeof(struct listening_socket_s), 10)) {  //  初始化一个socket list 数组，长度是10 
		return -1;
	}
