...
static int fpm_event_queue_add(struct fpm_event_queue_s **queue, struct fpm_event_s *ev) /* {{{ */
{
	...
	*queue = elt;
	...
	if (*queue == fpm_event_queue_fd && module->add) {   // 如果是 queue 队列会 会调用对应的 add 事件   比如 epoll  会给add赋予对应的函数指针
		module->add(ev);
	}
...
void fpm_event_loop(int err) /* {{{ */
{
	...
	fpm_event_set(&signal_fd_event, fpm_signals_get_fd(), FPM_EV_READ, &fpm_got_signal, NULL);  // 给父进程 增加监听 sp[0]的fd事件， 子进程有signal 过来会往sp[1] 写入信号处理  sp[0]监听这个信号
...
int fpm_event_add(struct fpm_event_s *ev, unsigned long int frequency) /* {{{ */
{
	...
	if (ev->flags & FPM_EV_READ) {  // epollin 监听
