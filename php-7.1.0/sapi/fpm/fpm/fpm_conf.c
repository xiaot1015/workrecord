...
{ // 检查fpm_globals的相关配置
	...
	if (0 > fpm_event_pre_init(fpm_global_config.events_mechanism)) { // 根据配置设置 事件模型  如 epoll、select、poll 等等
		return -1;
	}

	if (0 > fpm_conf_process_all_pools()) {  // 初始化进程 fpm_worker_pool_s  
		return -1;
	}
...
int fpm_conf_init_main(int test_conf, int force_daemon) /* {{{ */
{
	...
	ret = fpm_conf_load_ini_file(fpm_globals.config);  // load conf 文件
