...
<a href=\"http://php.net/install.windows\">the manual page</a>.</p>\n");
	...
	php_import_environment_variables = cgi_php_import_environment_variables;  // 设置 cgi 获取http 变量等参数  的函数

	/* library is already initialized, now init our request */
	request = fpm_init_request(fcgi_fd);   //  子进程 会走到这里   
	...
	zend_first_try {
		while (EXPECTED(fcgi_accept_request(request) >= 0)) {
			...
			init_request_info();  // 初始化 SG 中的request_info 相关

			fpm_request_info();    //// 获取fpm 请求相关的信息 并对他进行赋值
