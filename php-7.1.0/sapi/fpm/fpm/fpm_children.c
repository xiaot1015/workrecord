...
static struct fpm_child_s *fpm_resources_prepare(struct fpm_worker_pool_s *wp) /* {{{ */
{
	...
	c = fpm_child_alloc();    // 申请一个 fpm_child_s 进程 并将 scoreboard_i = -1;
	...
	c->wp = wp;
	c->fd_stdout = -1; c->fd_stderr = -1;   // 初始化 子进程 输输入输出

	if (0 > fpm_stdio_prepare_pipes(c)) {   // 检查 输出管道是否畅通  并且关闭了 子进程的 fd_stdout[0]fd_stdout[1]  fork时是 完全复制master进程的信息包括 输入输出fd
		fpm_child_free(c);
		return 0;
	}
...
static void fpm_child_resources_use(struct fpm_child_s *child) /* {{{ */
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		...
		fpm_scoreboard_free(wp->scoreboard);  // todo    问题
	}

	fpm_scoreboard_child_use(child->wp->scoreboard, child->scoreboard_i, getpid());
	fpm_stdio_child_use_pipes(child);
	fpm_child_free(child);   // weishenme free child
}
/* }}} */
